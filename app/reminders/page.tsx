...
export default function RemindersPage() {
  ...
  function checkReminders() {
    ...
    reminders.forEach((r) => {
      ...
      localStorage.setItem(
        "sehatsathi_reminders_fired",
        JSON.stringify(lastFiredRef.current)
      );

      if (Notification.permission === "granted") {
        const cfg = TYPE_CONFIG[r.type];
        new Notification(`${cfg.icon} ${r.label}`, {
          body: `SehatSathi reminder — ${r.time}`,
          icon: "/favicon.ico",
        });
      }
    });
  ...
  const grouped = {
    ...
    meal:     reminders.filter((r) => r.type === "meal"),
  ...
  return (
    <div className="min-h-screen pb-28">
      ...
      <div className="max-w-lg mx-auto px-4 mt-5 space-y-4">
          ...
          <div className="card border border-blue-100 bg-blue-50 flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-blue-600 mt-0.5">
                Allow notifications so reminders pop up even when you are on
                another tab.
              </p>
            </div>
            <button
              onClick={requestPermission}
              className="text-xs font-semibold text-white bg-blue-500 px-3 py-1.5 rounded-lg hover:bg-blue-600 transition whitespace-nowrap"
            >
              Allow
            </button>
          </div>
        ...
        {notifStatus === "granted" && (
          <div className="card bg-green-50 border border-green-100 flex items-center gap-2 py-2.5">
            <span className="text-green-500">✅</span>
            <p className="text-sm text-green-700 font-medium">
              Notifications are enabled — reminders will pop up on time.
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-2">
            {QUICK_REMINDERS.map((q) => {
              ...
              const exists = reminders.some(
                (r) => r.label === q.label && r.time === q.time
              );
              return (
                <button
                  key={q.label}
                  ...
                  className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-left transition-all ${
                    exists
                      ...
                      : `${cfg.bg} border-transparent hover:shadow-sm`
                  }`}
                >
                  <span className="text-xl">{cfg.icon}</span>
                  <div>
                    <p className={`text-xs font-semibold ${exists ? "text-gray-300" : cfg.color}`}>
                      {q.label}
                    </p>
        ...
        {!showForm ? (
          <button
            ...
            className="w-full border-2 border-dashed border-gray-200 rounded-xl py-3 text-sm text-gray-400 hover:border-blue-300 hover:text-blue-400 transition"
          >
            + Add Custom Reminder
          </button>
        ) : (
          <div className="card space-y-4">
            ...
            <input
              type="text"
              placeholder='e.g. "Take Metformin" or "Drink Water"'
              value={form.label}
            ...
            <button
              onClick={addReminder}
              disabled={!form.label.trim() || form.days.length === 0}
        ...
        {reminders.length === 0 ? (
          <div className="card text-center text-gray-400 py-10">
            ...
            <p className="text-sm mt-1">Use Quick Add above to get started</p>
          </div>
        ) : (
...
function ReminderRow({
  ...
  return (
    <div
      className={`card border flex items-center gap-3 py-3 transition-opacity ${
        reminder.active ? "opacity-100" : "opacity-50"
      }`}
    >
      ...
      <button
        ...
        className={`relative w-11 h-6 rounded-full transition-colors ${
          reminder.active ? "bg-blue-500" : "bg-gray-200"
        }`}
      >
        <span
