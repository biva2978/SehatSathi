...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Pick a daily tip based on today's date
    ...
    <div className="min-h-screen pb-28">
      ...
      <div className="max-w-lg mx-auto px-4 mt-5 space-y-4">
        ...
        {isNewUser && (
          <Link
            href="/profile"
            className="card border-2 border-dashed border-green-200 bg-green-50 flex items-center gap-3 hover:shadow-md transition-shadow"
          >
            ...
            <div>
              ...
              <p className="text-xs text-green-600 mt-0.5">
                Add your health conditions, goal &amp; budget →
              </p>
            </div>
          </Link>
        )}
...
function StatCard({
  icon, value, label, href, color, small,
}: {
  icon: string;
  value: string;
  label: string;
  ...
  return (
    <Link href={href} className="card border border-gray-100 text-center hover:shadow-md transition-shadow py-3">
      ...
      <div className={`font-bold ${small ? "text-sm" : "text-xl"} ${color}`}>
        {value}
      </div>
