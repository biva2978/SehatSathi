    ...
    <div className="min-h-screen pb-28 flex flex-col">
      <div className="bg-gradient-to-br from-violet-500 to-purple-500 px-5 pt-12 pb-5 text-white">
        ...
        <div className="flex gap-2 mt-4">
          <button onClick={() => setTab("analysis")} className={`px-4 py-1.5 rounded-full text-sm font-semibold transition ${tab === "analysis" ? "bg-white text-violet-600" : "bg-white/20 text-white"}`}>🔬 Analysis</button>
          <button onClick={() => setTab("chat")} className={`px-4 py-1.5 rounded-full text-sm font-semibold transition ${tab === "chat" ? "bg-white text-violet-600" : "bg-white/20 text-white"}`}>💬 Chat</button>
        </div>
      </div>

      {tab === "analysis" && (
        <div className="max-w-lg mx-auto w-full px-4 mt-4 pb-6 space-y-4">
          <button onClick={runAnalysis} disabled={analyzing} className="w-full py-3 rounded-xl font-semibold text-white text-sm transition disabled:opacity-50" style={{ background: "linear-gradient(135deg, #7c3aed, #a855f7)" }}>
          ...
          {analysisError && <p className="text-sm text-red-500 bg-red-50 px-4 py-3 rounded-xl">{analysisError}</p>}

          {!analysis && !analyzing && !analysisError && (
            <div className="card text-center py-10 space-y-2">
              <div className="text-4xl">🔬</div>
              <p className="font-semibold text-gray-700">Run your health analysis</p>
              <p className="text-sm text-gray-400">The AI agent will analyse all your medicines, health conditions, and goals to give you a personalised report.</p>
              {medicines.length === 0 && (
                <p className="text-xs text-orange-500 bg-orange-50 px-3 py-2 rounded-lg mt-2">Tip: Add medicines on the Medicines page for a fuller analysis.</p>
              )}
            </div>
          )}
            ...
            <>
              ...
              {analysis.urgentWarnings.length > 0 && (
                <div className="card border border-red-200 bg-red-50">
                  ...
                  <ul className="space-y-1">
                    {analysis.urgentWarnings.map((w, i) => (<li key={i} className="text-sm text-red-700 flex gap-2"><span className="mt-0.5 flex-shrink-0">🚨</span>{w}</li>))}
                  </ul>
                </div>
              )}
                ...
                <div className="card space-y-2">
                  ...
                  {analysis.interactions.map((item, i) => (
                    <div key={i} className={`rounded-xl px-3 py-2.5 ${item.severity === "warning" ? "bg-orange-50 border border-orange-100" : "bg-blue-50 border border-blue-100"}`}>
                      <p className={`text-xs font-semibold mb-0.5 ${item.severity === "warning" ? "text-orange-700" : "text-blue-700"}`}>{item.severity === "warning" ? "⚠️" : "ℹ️"} {item.title}</p>
              ...
              <div className="card space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {analysis.sideEffectsToWatch.map((s, i) => (<span key={i} className="bg-orange-50 text-orange-700 text-xs px-2.5 py-1 rounded-full border border-orange-100">{s}</span>))}
                </div>
              </div>
                ...
                <ul className="space-y-2">
                  {analysis.foodsToAvoid.map((item, i) => (
                    <li key={i} className="flex gap-2">
                      ...
                      <div><span className="text-sm font-medium text-gray-800">{item.food}</span><span className="text-xs text-gray-400 ml-1">— {item.reason}</span></div>
                    </li>
                  ))}
                </ul>
              </div>
                ...
                <ul className="space-y-2">
                  {analysis.foodsToEat.map((item, i) => (
                    <li key={i} className="flex gap-2">
                      ...
                      <div><span className="text-sm font-medium text-gray-800">{item.food}</span><span className="text-xs text-gray-400 ml-1">— {item.benefit}</span></div>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="card space-y-2">
                ...
                <ul className="space-y-2">
                  {analysis.lifestyleTips.map((tip, i) => (<li key={i} className="text-sm text-gray-700 flex gap-2"><span className="text-violet-400 mt-0.5 flex-shrink-0">•</span>{tip}</li>))}
                </ul>
              </div>

              <p className="text-xs text-center text-gray-400 pb-2">⚠️ AI-generated analysis. Always consult your doctor before making health decisions.</p>
            </>
          )}
        </div>
      )}
            ...
            <div className="space-y-4">
              <div className="card bg-violet-50 border border-violet-100 flex gap-3 items-start">
                ...
                <div>
                  ...
                  <p className="text-xs text-violet-600 mt-0.5">Ask me anything — I&apos;ll give personalised answers based on your health data.</p>
                </div>
              </div>
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide px-1">Try asking…</p>
              <div className="grid grid-cols-1 gap-2">
                {SUGGESTIONS.map((s) => (<button key={s} onClick={() => send(s)} className="text-left px-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm text-gray-600 hover:border-violet-300 hover:bg-violet-50 transition">{s}</button>))}
              </div>
            </div>
          )}
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
                msg.role === "user" ? "bg-violet-500 text-white rounded-br-sm" : "bg-white border border-gray-100 text-gray-800 rounded-bl-sm shadow-sm"
              }`}>
      ...
      {tab === "chat" && (
        <div className="fixed bottom-16 left-0 right-0 bg-white border-t border-gray-100 px-4 py-3 z-40">
          <div className="max-w-lg mx-auto flex gap-2">
            <input type="text" placeholder="Ask about health, medicines, diet…" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send(input)} className="flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300" />
            <button onClick={() => send(input)} disabled={!input.trim() || loading} className="bg-violet-500 hover:bg-violet-600 text-white px-4 py-2.5 rounded-xl font-semibold text-sm disabled:opacity-40 transition">Send</button>
          </div>
        </div>
      )}
