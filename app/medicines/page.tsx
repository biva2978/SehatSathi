...
export default function MedicinesPage() {
  ...
  async function handleBulkAdd() {
    ...
    if (results.length > 0) {
      await saveMedicines([...results, ...medicines]);
    }
      ...
      <div className="max-w-lg mx-auto px-4 mt-5 space-y-4">
        ...
        <div className="card space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex rounded-lg border border-gray-200 overflow-hidden text-xs">
              <button
                onClick={() => { setMode("single"); setBulkStatuses([]); }}
                className={`px-3 py-1.5 font-medium transition ${mode === "single" ? "bg-green-500 text-white" : "bg-white text-gray-500"}`}
              >
                Single
              </button>
              <button
                onClick={() => { setMode("bulk"); setError(""); }}
                className={`px-3 py-1.5 font-medium transition ${mode === "bulk" ? "bg-green-500 text-white" : "bg-white text-gray-500"}`}
              >
                Multiple
              </button>
            </div>
          ...
          {mode === "single" ? (
            <>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {FREQ_OPTIONS.map((f) => (
                    <button key={f} onClick={() => setFrequency(f)} className={`px-3 py-1 rounded-full text-xs font-medium border transition-all ${frequency === f ? "bg-green-500 text-white border-green-500" : "bg-white text-gray-500 border-gray-200 hover:border-green-300"}`}>{f}</button>
                  ))}
                </div>
              </div>
              {error && <p className="text-sm text-red-500 bg-red-50 px-3 py-2 rounded-lg">{error}</p>}
              <button onClick={handleAdd} disabled={loading || !inputName.trim()} className="w-full btn-primary py-2.5 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <>
              <textarea
                placeholder={"Enter one medicine per line, or separate with commas:\nMetformin\nFolic Acid\nOmeprazole, Amlodipine"}
                value={bulkInput}
              ...
              >
                {bulkLoading ? "Looking up all medicines…" : "Add All Medicines"}
              </button>
        ...
        {medicines.length === 0 ? (
          <div className="card text-center text-gray-400 py-10">
            ...
            <p className="text-sm mt-1">Use Single or Multiple mode above to add</p>
          </div>
        ) : (
      ...
      <div className="flex items-start justify-between gap-2">
        <button onClick={onToggle} className="flex-1 text-left">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="font-semibold text-gray-800 text-sm">{med.info?.name ?? med.inputName}</p>
